...
namespace ORBSlam
{
	...
	{
		int nmatches = 0;
		vnMatches12 = vector<int>(F1.GetUnKeyPts().size(), -1);
		...
		vector<int> vMatchedDistance(F2.GetUnKeyPts().size(), INT_MAX);
		vector<int> vnMatches21(F2.GetUnKeyPts().size(), -1);

		for (size_t i1 = 0, iend1 = F1.GetUnKeyPts().size(); i1 < iend1; i1++)
		{
			cv::KeyPoint kp1 = F1.GetUnKeyPts()[i1];
			...
			if (bestDist <= TH_LOW)
			{
				if (bestDist < (float)bestDist2*mfNNratio)
				{
					...
					{
						float rot = F1.GetUnKeyPts()[i1].angle - F2.GetUnKeyPts()[bestIdx2].angle;
						if (rot < 0.0)
							rot += 360.0f;
		...
		for (size_t i1 = 0, iend1 = vnMatches12.size(); i1 < iend1; i1++)
			if (vnMatches12[i1] >= 0)
				vbPrevMatched[i1] = F2.GetUnKeyPts()[vnMatches12[i1]].pt;
